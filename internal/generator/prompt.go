...
// PromptCLIConfig interactively prompts user for CLI app configuration
func PromptCLIConfig() (*CLIConfig, error) {
	config := NewDefaultCLIConfig()

	fmt.Println("\n🚀 Create Multi-App Project with CLI Application")
	fmt.Println("──────────────────────────────────────────────────")
	...
	if err := survey.AskOne(&survey.Input{
		Message: "Application name:",
		Default: "my-cli",
		Help:    "Use kebab-case, e.g., my-cli, admin-cli",
	}, &config.AppName, survey.WithValidator(survey.Required)); err != nil {
		return nil, err
	}
	...
	if err := survey.AskOne(&survey.Input{
		Message: "Application description:",
		Default: fmt.Sprintf("%s CLI Tool", ToPascalCase(config.AppName)),
	}, &config.Description); err != nil {
		return nil, err
	}
...
// PromptCronConfig interactively prompts user for Cron app configuration
func PromptCronConfig() (*CronConfig, error) {
	config := NewDefaultCronConfig()

	fmt.Println("\n🚀 Create Multi-App Project with Cron Application")
	fmt.Println("──────────────────────────────────────────────────")
	...
	if err := survey.AskOne(&survey.Input{
		Message: "Application name:",
		Default: "my-cron",
		Help:    "Use kebab-case, e.g., my-cron, task-scheduler",
	}, &config.AppName, survey.WithValidator(survey.Required)); err != nil {
		return nil, err
	}
	...
	if err := survey.AskOne(&survey.Input{
		Message: "Application description:",
		Default: fmt.Sprintf("%s Scheduled Tasks", ToPascalCase(config.AppName)),
	}, &config.Description); err != nil {
		return nil, err
	}
	...
	fmt.Println("  │       ├── command/  (start, run)")
	fmt.Println("  │       └── handler/  (demo_handler)")
	fmt.Println("  ├── domains/")
	fmt.Println("  ├── pkg/")
	fmt.Println("  └── scripts/")
	fmt.Println("═══════════════════════════════════════════════════")
