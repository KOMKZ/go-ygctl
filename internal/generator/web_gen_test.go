...
func TestResolveWebFields_Defaults(t *testing.T) {
	...
	// username：string 默认列+搜索+表单，required 派生校验
	u := byName["username"]
	if !u.ColumnShow || !u.SearchShow || !u.FormShow || u.FormType != "input" {
		t.Errorf("username defaults = %+v, want column+search+form input", u)
	}
	...
	// role：enum → 列带 options + select 表单/搜索
	r := byName["role"]
	...
	if r.FormType != "select" || r.SearchType != "select" || !r.SearchShow {
		t.Errorf("role form/search = %q/%q (show=%v), want select/select/true", r.FormType, r.SearchType, r.SearchShow)
	}
...
func TestBuildWebGenData_Fragments(t *testing.T) {
	...
	ui := &UIDef{Style: "dialog", Menu: UIMenu{Title: "管理员管理", Icon: "Users"}}
